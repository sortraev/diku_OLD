...
public class StockManagerTest {
	...
	private static final int TEST_ISBN  = 30345650;
	private static final int TEST_ISBN1 = TEST_ISBN + 1;
	private static final int TEST_ISBN2 = TEST_ISBN + 2;
	...
	public StockBook getDefaultBook2() {
		return new ImmutableStockBook(TEST_ISBN1, "foo title",
						"foo author", (float) 42, NUM_COPIES, 0, 0, 0, false);
	}
	public StockBook getDefaultBook3() {
		return new ImmutableStockBook(TEST_ISBN2, "bar title",
	...
	@Test
	public void testBookInDemandAfterFailedPurchase() throws BookStoreException {
		// assert no books in demand at start.
		List<StockBook> expected = new ArrayList<>(); // empty list.
		assertEquals(expected, storeManager.getBooksInDemand());

		// buy 6 copies of default book. this should fail,
		// since there should only be 5 copies available.
		...
		// assert that book is now in demand.
		expected.add(getDefaultBook());
		assertEquals(expected, storeManager.getBooksInDemand());
	}

	@Test
	public void testBookNotInDemandAfterSuccessfulPurchase() throws BookStoreException {
		// assert no books in demand at start.
		List<StockBook> expected = new ArrayList<>(); // empty list.
		assertEquals(expected, storeManager.getBooksInDemand());

		// buy 4 copies of default book. this should be fine, since there are 5 in stock.
		Set<BookCopy> booksToBuy = new HashSet<>();
		booksToBuy.add(new BookCopy(TEST_ISBN, NUM_COPIES-1));
		client.buyBooks(booksToBuy);

		// if the purchase was successful, then the book should still not be in demand.
		assertEquals(expected, storeManager.getBooksInDemand());
	}

	@Test
	public void testSomeBooksInDemandSomeNot() throws BookStoreException {
		// assert no books in demand at start.
		List<StockBook> expected = new ArrayList<>(); // empty.
		assertEquals(expected, storeManager.getBooksInDemand());


		// add some books to the bookstore.
    StockBook book0 = getDefaultBook();
		StockBook book2 = getDefaultBook2();
		StockBook book3 = getDefaultBook3();
		Set<StockBook> booksToAdd = new HashSet<>();
		booksToAdd.add(book2);
		booksToAdd.add(book3);
		storeManager.addBooks(booksToAdd);

		// make two valid purchases and one invalid.
		Set<BookCopy> booksToBuy = new HashSet<>();
		booksToBuy.add(new BookCopy(TEST_ISBN, NUM_COPIES));    // this should be available.
		booksToBuy.add(new BookCopy(TEST_ISBN1, NUM_COPIES+1)); // this should not be available.
		booksToBuy.add(new BookCopy(TEST_ISBN2, 1));            // this should be available.
		try {
			client.buyBooks(booksToBuy);
		}
		catch (BookStoreException ignored) {
		...
		// fetch list of books now in demand.
		List<StockBook> nowInDemand = storeManager.getBooksInDemand();

		// assert that book2 is now in demand.
		expected.add(book2);
		assertEquals(expected, nowInDemand);

		// this is already asserted by the above assertEquals, but nevertheless,
		// assert that neither book0 nor book3 is in demand.
		assertFalse(nowInDemand.contains(book0));
		assertFalse(nowInDemand.contains(book3));
	}
