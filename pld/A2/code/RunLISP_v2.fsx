...
let rec eval inLambda s localEnv =
  match s with
  | Nil -> Nil
  ...
  | Cons (Symbol "if", es) ->
       match es with
       | Nil -> Nil
       | Cons (e1, Nil) -> eval inLambda e1 localEnv
       | Cons (e1, Cons (e2, ess)) ->
           if eval inLambda e1 localEnv <> Nil
           then eval inLambda e2 localEnv
           else eval inLambda (Cons (Symbol "if", ess)) localEnv
  ...
  | Cons (Symbol "define", Cons (Symbol x, Cons (e, Nil))) ->
      ...
      if List.contains x predefined then
        ...
        (updateGlobal x (eval inLambda e localEnv); Symbol x)
  | Cons (Symbol "cons", Cons (e1, Cons (e2, Nil))) -> // builds a pair
      Cons (eval inLambda e1 localEnv, eval inLambda e2 localEnv)
  ...
  | Cons (e1, args) -> // function application
        applyFun inLambda (eval inLambda e1 localEnv, evalList inLambda args localEnv, localEnv)
...
and applyLambda inLambda (fnc, pars, localEnv) =
  match fnc with
    | Closure (Cons (Symbol "lambda", rules), closureEnv) ->
        try tryRules inLambda rules pars closureEnv
        with 
        | Lerror message ->
        ...
        | ReturnStmException s -> eval inLambda s localEnv

    | Cons (Symbol "lambdaD", rules) ->
        try tryRules inLambda rules pars localEnv
        with 
        | Lerror message ->
          raise (Lerror (message + "\n! when applying "
...
and evalList inLambda es localEnv =
  match es with
  | Nil -> Nil
  | Cons (e1, es1) -> Cons (eval inLambda e1 localEnv, evalList inLambda es1 localEnv)
...
and applyBinop inLambda x (v, w, localEnv) =
  match (x, v, w) with
  | ("cons", _, _) -> Cons (v, w)
  | ("apply", _, _) -> applyFun inLambda (v, w, localEnv)
...
and tryRules inLambda rules pars localEnv =
  match rules with
  | Cons (p, Cons (e, rules1)) ->
      match matchPattern p pars with
      | Some env -> eval inLambda e (env @ localEnv)
      | None     -> tryRules inLambda rules1 pars localEnv
  | Nil -> raise (Lerror ("No patterns matched arguments "
                          + showSexpIndent pars 29 29))
  | _ ->   raise (Lerror ("Malformed rules " + showSexpIndent rules 16 16))
