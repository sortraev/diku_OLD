...
and applyFun (fnc, pars, localEnv) =
  match fnc with
    ...
    | Closure (Cons (Symbol "lambda", _), _) -> applyLambda (fnc, pars, localEnv)
    | Cons (Symbol "lambdaD", _)             -> applyLambda (fnc, pars, localEnv)
  ...
  match fnc with
    ...
    | Cons (Symbol "lambdaD", rules) ->
      ...
      // using the current environment
        try tryRules rules pars localEnv
        with 
        | Lerror message ->
          raise (Lerror (message + "\n! when applying "
...
and tryRules rules pars localEnv =
  ...
  | Cons (p, Cons (e, rules1)) ->
      ...
      | None     -> tryRules rules1 pars localEnv
