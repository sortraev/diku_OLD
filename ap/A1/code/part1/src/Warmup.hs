...
newtype RWSP a = RWSP {runRWSP :: ReadData -> StateData -> (a, WriteData, StateData)}
...
instance Monad RWSE where
  return a = RWSE $ \_ s -> return (a, mempty, s)

  m >>= f = RWSE $ \r s -> runRWSE m r s >>=
          \(a,  w,  s2) -> runRWSE (f a) r s2 >>=
          \(a2, w2, s3) -> return (a2, w <> w2, s3)
...
allTests = [testP, testP', testE, testE2, testE', testE2']
